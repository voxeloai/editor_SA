...
class PointerController {
  ...
  constructor(camera: Camera, target: HTMLElement) {
    const orbit = (dx: number, dy: number) => {
      ...
      const elev =
        camera.elevation - dy * camera.scene.config.controls.orbitSensitivity;
    ...
    const pan = (x: number, y: number, dx: number, dy: number) => {
      ...
      const distance =
        (camera.distanceTween.value.distance * camera.sceneRadius) /
        camera.fovFactor;
    ...
    const zoom = (amount: number) => {
      camera.setDistance(
        camera.distance -
          (camera.distance * 0.999 + 0.001) *
            amount *
            camera.scene.config.controls.zoomSensitivity,
        2,
      );
    };
    ...
    const pointermove = (event: PointerEvent) => {
      if (event.pointerType === "mouse") {
        ...
        if (touches.length === 1) {
          ...
          const touch =
            touches[touches.map((t) => t.id).indexOf(event.pointerId)];
          touch.x = event.offsetX;
          touch.y = event.offsetY;
