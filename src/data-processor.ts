...
import {
  vertexShader as boundVS,
  fragmentShader as boundFS,
} from "./shaders/bound-shader";
import {
  vertexShader as intersectionVS,
  fragmentShader as intersectionFS,
} from "./shaders/intersection-shader";
import {
  vertexShader as positionVS,
  fragmentShader as positionFS,
} from "./shaders/position-shader";
...
class DataProcessor {
    ...
    this.getBoundResources = (() => {
      ...
      return (width: number) => {
        ...
        if (!minTexture || minTexture.width !== width) {
          ...
          minTexture = createTexture(
            "calcBoundMin",
            width,
            1,
            PIXELFORMAT_RGBA32F,
          );
          maxTexture = createTexture(
            "calcBoundMax",
            width,
            1,
            PIXELFORMAT_RGBA32F,
  ...
  intersect(
    options: MaskOptions | RectOptions | SphereOptions | BoxOptions,
    splat: Splat,
  ) {
    const { device } = this;
    const { scope } = device;
    ...
    const transformA = (splat.entity.gsplat.instance.resource as GSplatResource)
      .transformATexture;
    ...
    if (boxOptions.box) {
      resolve(scope, {
        mode: 3,
        box_params: [boxOptions.box.x, boxOptions.box.y, boxOptions.box.z, 0],
        aabb_params: [
  ...
  calcBound(splat: Splat, boundingBox: BoundingBox, onlySelected: boolean) {
    ...
    glDevice.gl.readPixels(
      ...
      transformA.width,
      1,
      resources.minTexture.impl._glFormat,
      resources.minTexture.impl._glPixelType,
      resources.minData,
    );
    ...
    glDevice.gl.readPixels(
      ...
      transformA.width,
      1,
      resources.maxTexture.impl._glFormat,
      resources.maxTexture.impl._glPixelType,
      resources.maxData,
    );
    glDevice.updateEnd();
  ...
  calcPositions(splat: Splat) {
    ...
    // allocate resources
    const resources = this.getPositionResources(
      transformA.width,
      transformA.height,
      numSplats,
    );
