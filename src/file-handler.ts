...
const initFileHandler = (
  ...
  const importSog = async (files: ImportFile[], animationFrame: boolean) => {
    const meta = files.findIndex(
      (f) => f.filename.toLowerCase() === "meta.json",
    );
    const urls = files.map(
      (file) =>
        (file.contents && URL.createObjectURL(file.contents)) ??
        file.url ??
        file.filename,
    );
  ...
  const importLcc = async (files: ImportFile[], animationFrame: boolean) => {
    try {
      const meta = files.findIndex((f) =>
        f.filename.toLowerCase().endsWith(".lcc"),
      );
    ...
    if (isPlySequence(filenames)) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (filename.endsWith(".ssproj")) {
          ...
          await events.invoke(
            "doc.load",
            files[i].contents ?? (await fetch(files[i].url)).arrayBuffer(),
            files[i].handle,
          );
        } else if (
          [".ply", ".splat", ".sog"].some((ext) => filename.endsWith(ext))
        ) {
  ...
  if (!window.showOpenFilePicker) {
    ...
    fileSelector.setAttribute(
      "accept",
      ".ply,.splat,meta.json,.json,.webp,.ssproj,.sog,.lcc,.bin,.txt",
    );
  ...
  events.function(
    "scene.export",
    async (exportType: "ply" | "splat" | "viewer") => {
      const splats = getSplats();

      const hasFilePicker = !!window.showSaveFilePicker;

      // show viewer export options
      const options = (await events.invoke(
        "show.exportPopup",
        exportType,
        splats.map((s) => s.name),
        !hasFilePicker,
      )) as SceneExportOptions;
      ...
      const fileType =
        exportType === "viewer"
          ? options.viewerExportSettings.type === "zip"
            ? "packageViewer"
            : "htmlViewer"
          : exportType === "ply"
            ? options.compressedPly
              ? "compressedPly"
              : "ply"
            : "splat";
  ...
  events.function(
    ...
    async (
      ...
      try {
        ...
        const { filename, splatIdx, serializeSettings, viewerExportSettings } =
          options;

        const writer = stream
          ? new FileStreamWriter(stream)
          : new DownloadWriter(filename);

        try {
          const splats =
            splatIdx === "all" ? getSplats() : [getSplats()[splatIdx]];
