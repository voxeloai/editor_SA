...
const fetchUser = async () => {
  // fetch user data from api\iframe?
  ...
  static async create(publishSettings: PublishSettings) {
    ...
    result.close = async () => {
      ...
      const completeResult = await fetch(
        `${user.apiServer}/upload/complete-upload`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${user.token}`,
      ...
      if (!completeResult.ok) {
        throw new Error(
          `failed to complete upload (${completeResult.statusText})`,
        );
      }
      ...
      const doRepublish = () =>
        fetch(
          `${user.apiServer}/splats/${publishSettings.overwriteId}/republish`,
          {
            method: "PUT",
            body: JSON.stringify({
              s3Key: startJson.key,
      ...
      const publishResponse = await (publishSettings.overwriteId
        ? doRepublish()
        : doPublish());
...
const registerPublishEvents = (events: Events) => {
  console.log("[publish.ts] registerPublishEvents");

  //   events.function("publish.userStatus", async () => {
  //     return { user: { username: "iframe" }, scenes: [] };
  //   });

  // here call iframe api to publish
  events.function("scene.publish", async () => {
    await events.invoke("scene.saveScan");
  });
  // TODO: remove or get blob ?
  ...
  //       await serializePly(
  //         splats,
  //         publishSettings.serializeSettings,
  //         gzipWriter,
  //         progressFunc
  //       );
