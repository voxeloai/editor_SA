...
const registerRenderEvents = (scene: Scene, events: Events) => {
  ...
  events.function(
    "render.offscreen",
    async (width: number, height: number): Promise<Uint8Array> => {
      try {
        ...
        await workRenderTarget.colorBuffer.read(0, 0, width, height, {
          renderTarget: workRenderTarget,
          data,
        });
  ...
  events.function(
    ...
    async (
      ...
      try {
        ...
        if (!fileStream) {
          downloadFile(
            (output.target as BufferTarget).buffer,
            `${removeExtension(splats[0]?.name ?? "supersplat")}.${fileExtension}`,
          );
        }
