...
import { Color, GSplatData, Mat3, Mat4, Quat, Vec3 } from "playcanvas";

import { SHRotation } from "./sh-utils";
import { Splat } from "./splat";
import { State } from "./splat-state";
import { version } from "../package.json";
import { BufferWriter, ProgressWriter } from "./serialize/writer";
import type { Writer } from "./serialize/writer";
...
const getCommonProps = (splats: Splat[]) => {
  ...
  for (let i = 0; i < splats.length; ++i) {
    const properties = splats[i].splatData
      .getElement("vertex")
      .properties.filter((p: any) => p.storage);
...
const calcSHBands = (data: Set<string>) => {
  return (
    { "9": 1, "24": 2, "-1": 3 }[shNames.findIndex((v) => !data.has(v))] ?? 0
  );
};
...
class SplatTransformCache {
  ...
  constructor(splat: Splat, keepWorldTransform = false) {
    ...
    >();
    const indices =
      splat.transformTexture.getSource() as unknown as Uint32Array;
    ...
    const read = (splat: Splat, i: number) => {
      ...
      if (
        ...
        const invSig = (value: number) =>
          value <= 0 ? -400 : value >= 1 ? 400 : -Math.log(1 / value - 1);
...
class Chunk {
  ...
  pack() {
    ...
    const pack111011 = (x: number, y: number, z: number) => {
      return (
        (packUnorm(x, 11) << 21) | (packUnorm(y, 10) << 11) | packUnorm(z, 11)
      );
    };
...
const serializePlyCompressed = async (
  splats: Splat[],
  ...
  const shHeader = outputSHBands
    ? [
        ...
        new Array(outputSHCoeffs * 3)
          .fill("")
          .map((_, i) => `property uchar f_rest_${i}`),
      ].flat()
    : [];
