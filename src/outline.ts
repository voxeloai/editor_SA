...
class Outline extends Element {
  ...
  add() {
    ...
    this.scene.events.on("selection.changed", (splat: Splat, prev: Splat) => {
      if (prev) {
        prev.entity.gsplat.layers = prev.entity.gsplat.layers.filter(
          (id) => id !== layerId,
        );
      }
      if (splat) {
        splat.entity.gsplat.layers = splat.entity.gsplat.layers.concat([
          layerId,
        ]);
      }
    });
    ...
    this.entity.camera.on(
      "postRenderLayer",
      (layer: Layer, transparent: boolean) => {
        if (
          !this.entity.enabled ||
          layer !== this.scene.overlayLayer ||
          !transparent
        ) {
          return;
        }
        ...
        alphaCutoffId.setValue(
          events.invoke("camera.mode") === "rings" ? 0.0 : 0.4,
        );
