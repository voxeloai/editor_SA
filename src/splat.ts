...
class Splat extends Element {
  ...
  constructor(asset: Asset, orientation: Vec3) {
    ...
    instance.meshInstance.calculateSortDistance = (
  ...
  onPreRender() {
    ...
    const selected =
      this.scene.camera.renderOverlays && events.invoke("selection") === this;
    ...
    const selectionAlpha =
      selected && !events.invoke("view.outlineSelection")
        ? this.selectionAlpha
        : 0;
  ...
  get worldBound() {
    ...
    if (this.worldBoundDirty) {
      ...
      worldBound.setFromTransformedAabb(
        this.localBound,
    ...
    switch (mode) {
      ...
      case "boundCenter":
        entity
          .getLocalTransform()
          .transformPoint(
            (selection ? this.selectionBound : this.localBound).center,
            vec,
          );
