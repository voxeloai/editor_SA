...
class PublishSettingsDialog extends Container {
  ...
  constructor(events: Events, args = {}) {
    ...
    const headerText = new Label({
      id: "text",
      text: localize("popup.publish.header"),
    });
    ...
    const overwriteLabel = new Label({
      class: "label",
      text: localize("popup.publish.to"),
    });
    ...
    const titleLabel = new Label({
      class: "label",
      text: localize("popup.publish.title"),
    });
    ...
    const descLabel = new Label({
      class: "label",
      text: localize("popup.publish.description"),
    });
    ...
    const listLabel = new Label({
      class: "label",
      text: localize("popup.publish.listed"),
    });
    ...
    const animationLabel = new Label({
      class: "label",
      text: localize("popup.export.animation"),
    });
    ...
    const reset = (hasPoses: boolean, overwriteList: string[]) => {
      ...
      overwriteSelect.options = [
        {
          v: "0",
          t: localize("popup.publish.new-scene"),
        },
      ...
      animationSelect.disabledOptions = hasPoses
        ? {}
        : { track: animationSelect.options[1].t };
      ...
      return new Promise<PublishSettings>((resolve) => {
        ...
        onOK = () => {
          ...
          resolve({
            ...
            overwriteId:
              overwriteSelect.value !== "0"
                ? userStatus.scenes[parseInt(overwriteSelect.value, 10) - 1].id
                : undefined,
          });
        };
      }).finally(() => {
