  ...
  constructor(events: Events, tooltips: Tooltips, args = {}) {
    ...
    const rebuild = () => {
      ...
      const labelStep = Math.max(
        1,
        Math.floor(numFrames / Math.max(1, Math.floor(width / 50))),
      );
      ...
      frameFromOffset = (offset: number) => {
        return Math.max(
          ...
          Math.min(
            numFrames - 1,
            Math.floor(((offset - padding) / width) * (numFrames - 1)),
          ),
        );
      };
      ...
      const createKey = (value: number) => {
        ...
        label.addEventListener("pointermove", (event: PointerEvent) => {
          if (dragging) {
            toFrame = frameFromOffset(
              parseInt(label.style.left, 10) + event.offsetX,
            );
...
class TimelinePanel extends Container {
  constructor(events: Events, tooltips: Tooltips, args = {}) {
    ...
    const skip = (dir: "forward" | "back") => {
      const orderedKeys = (events.invoke("timeline.keys") as number[])
        .map((frame, index) => {
          return { frame, index };
        })
      ...
      if (orderedKeys.length > 0) {
        ...
        const nextKey = orderedKeys.findIndex((k) =>
          dir === "back" ? k.frame >= frame : k.frame > frame,
        );
        ...
        if (nextKey === -1) {
          events.fire(
            "timeline.setFrame",
            orderedKeys[dir === "back" ? l - 1 : 0].frame,
          );
        } else {
          events.fire(
            "timeline.setFrame",
            orderedKeys[dir === "back" ? (nextKey + l - 1) % l : nextKey].frame,
          );
        }
      } else {
        ...
        if (dir === "back") {
          ...
          events.fire(
            "timeline.setFrame",
            events.invoke("timeline.frames") - 1,
          );
        }
      }
    };
    ...
    removeKey.on("click", () => {
      const index = events
        .invoke("timeline.keys")
        .indexOf(events.invoke("timeline.frame"));
      if (index !== -1) {
    ...
    const canDelete = (frame: number) =>
      events.invoke("timeline.keys").includes(frame);
    ...
    tooltips.register(
      removeKey,
      localize("tooltip.timeline.remove-key"),
      "top",
    );
    ...
    tooltips.register(
      smoothness,
      localize("tooltip.timeline.smoothness"),
      "top",
    );
  }
}
