...
class DataPanel extends Panel {
  constructor(events: Events, args = {}) {
    ...
    const populateDataSelector = (splat: Splat) => {
      ...
      const dataProps = splat.splatData
        .getElement("vertex")
      ...
      const suppressedProps = ["state", "transform"].concat(
        new Array(45).fill("").map((_, i) => `f_rest_${i}`),
      );
      const allProps = dataProps
        .concat(derivedProps)
        .filter((p) => !suppressedProps.includes(p));
    ...
    const splatsValue = dataLabel(
      controls,
      localize("panel.splat-data.totals.splats"),
    );
    const selectedValue = dataLabel(
      controls,
      localize("panel.splat-data.totals.selected"),
    );
    const lockedValue = dataLabel(
      controls,
      localize("panel.splat-data.totals.locked"),
    );
    const deletedValue = dataLabel(
      controls,
      localize("panel.splat-data.totals.deleted"),
    );
      ...
      if (dataFunc && data) {
        ...
        switch (dataSelector.value) {
          case "volume": {
            ...
            func = (i) =>
              scaleFunc(sx[i]) * scaleFunc(sy[i]) * scaleFunc(sz[i]);
            break;
          }
          case "distance": {
