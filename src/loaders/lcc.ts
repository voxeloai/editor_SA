...
const decodeSplat = (
  ...
  // decode color
  unitProperties.property_f_dc_0[i] = InvSH0ToColor(
    dataView.getUint8(off + 12) / 255.0,
  );
  unitProperties.property_f_dc_1[i] = InvSH0ToColor(
    dataView.getUint8(off + 13) / 255.0,
  );
  unitProperties.property_f_dc_2[i] = InvSH0ToColor(
    dataView.getUint8(off + 14) / 255.0,
  );
  unitProperties.property_opacity[i] = InvSigmoid(
    dataView.getUint8(off + 15) / 255.0,
  );

  // decode scale
  const scaleMin: Vec3 = compressInfo.compressedScaleMin;
  const scaleMax: Vec3 = compressInfo.compressedScaleMax;
  unitProperties.property_scale_0[i] = InvLinearScale(
    mix(scaleMin.x, scaleMax.x, dataView.getUint16(off + 16, true) / 65535.0),
  );
  unitProperties.property_scale_1[i] = InvLinearScale(
    mix(scaleMin.y, scaleMax.y, dataView.getUint16(off + 18, true) / 65535.0),
  );
  unitProperties.property_scale_2[i] = InvLinearScale(
    mix(scaleMin.z, scaleMax.z, dataView.getUint16(off + 20, true) / 65535.0),
  );
  ...
  if (isHasSH && shDataView && unitProperties_f_rest) {
    const shOff = off * 2;
    const SHValues = Array.from({ length: 15 }, (_, idx) =>
      shDataView.getUint32(shOff + idx * 4, true),
    );
    const { compressedSHMin, compressedSHMax } = compressInfo;
    const vecSHValues = SHValues.map((sh) =>
      mixVec3(compressedSHMin, compressedSHMax, DecodePacked_11_10_11(sh)),
    );
...
const processUnit = async (ctx: ProcessUnitContext) => {
  ...
  if (isHasSH) {
    const shSource = await createReadSource(
      shFile,
      offset * 2,
      offset * 2 + size * 2,
    );
  ...
  const unitProperties_f_rest = isHasSH
    ? Array.from({ length: 45 }, () => new Float32Array(unitSplats))
    : null;
  ...
  for (let i = 0; i < unitSplats; i++) {
    decodeSplat(
      ...
      compressInfo,
      unitProperties,
      unitProperties_f_rest,
      isHasSH,
    );
  }

  for (const key of floatProps) {
    properties[`property_${key}`].set(
      unitProperties[`property_${key}`],
      propertyOffset,
    );
  }
...
const deserializeFromLcc = async (param: LccParam) => {
  ...
  const properties_f_rest = isHasSH
    ? Array.from({ length: 45 }, () => createStorage(totalSplats))
    : null;
  ...
  const gsplatData = new GSplatData([
    {
      ...
      properties: [
          ...
          : []),
        {
          type: "float",
          name: "state",
          storage: new Uint8Array(totalSplats),
          byteSize: 4,
        },
      ],
    },
...
const loadLcc = async (assetSource: AssetSource) => {
  ...
  const isHasSH: boolean =
    meta.fileType === "Quality" && !!assetSource.mapFile("shcoef.bin");
  ...
  const indexSource = new ReadSource(
    indexFile.contents ?? indexFile.url ?? indexFile.filename,
  );
