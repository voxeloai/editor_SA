...
class Camera extends Element {
  ...
  set ortho(value: boolean) {
    if (value !== this.ortho) {
      this.entity.camera.projection = value
        ? PROJECTION_ORTHOGRAPHIC
        : PROJECTION_PERSPECTIVE;
  ...
  add() {
    ...
    if (this.scene.config.camera.debugRender) {
      this.entity.camera.setShaderPass(
        `debug_${this.scene.config.camera.debugRender}`,
      );
    }
  ...
  remove() {
    ...
    this.entity.camera.layers = this.entity.camera.layers.filter(
      (layer) => layer !== this.scene.shadowLayer.id,
    );
  ...
  rebuildRenderTargets() {
    ...
    const format = this.scene.events.invoke("camera.highPrecision")
      ? PIXELFORMAT_RGBA16F
      : PIXELFORMAT_RGBA8;
    ...
    const workColorBuffer = createTexture(
      "workColor",
      width,
      height,
      PIXELFORMAT_RGBA8,
  ...
  onUpdate(deltaTime: number) {
    ...
    camera.orthoHeight =
      ...
      (camera.horizontalFov
        ? this.scene.targetSize.height / this.scene.targetSize.width
        : 1);
  ...
  get fovFactor() {
    ...
    const aspect =
      width && height
        ? this.entity.camera.horizontalFov
          ? height / width
          : width / height
        : 1;
    const fov =
      2 * Math.atan(Math.tan(this.fov * math.DEG_TO_RAD * 0.5) * aspect);
  ...
  pickPrep(splat: Splat, op: "add" | "remove" | "set") {
    ...
    device.scope
      .resolve("pickMode")
      .setValue(["add", "remove", "set"].indexOf(op));
  ...
  pickRect(x: number, y: number, width: number, height: number) {
    ...
    device.readPixels(
      x,
      this.picker.renderTarget.height - y - height,
      width,
      height,
      pixels,
    );
